...
/*
    Extensión layerSwitcher (V. 3.1.8):
    "https://viglino.github.io/ol-ext/examples/control/map.switcher.html" modificada para adaptarla a necesidades propias.
*/

import ol_ext_inherits from 'ol-ext/util/ext'  // modificado =====> TSIG_OL
...
import { OlTsLayerScale as ts_layer_scale } from '../../../../src/layer'  // =====> TSIG_OL
 ...
 *  @param {function} options.oninfo callback on click on info button, if none no info button is shown DEPRECATED: use on(info) instead
  * @param {function} options.onedit callback on click on edit button, // =====> TSIG_OL
 *  @param {function} options.onselect callback on click on li panel of the layer   // =====> TSIG_OL
...
var ol_control_LayerSwitcher = function(options) {
  ...
  this.onedit = (typeof (options.onedit) == "function" ? options.onedit: null); // =====> TSIG_OL
  this.onselect = (typeof (options.onselect) == "function" ? options.onselect: null); // =====> TSIG_OL
...
ol_control_LayerSwitcher.prototype.drawList = function(ul, collection) {
  ...
  // layer title selectable // ====> TSIG_OL
  var setWorkLayer = function(e) {
    e.stopPropagation();
    e.preventDefault();
    var l = self._getLayerForLI(this);
    self.onselect(l); 
    self.dispatchEvent({ type: "ts_selected", layer: l });
  };
  // Edit button click    // ====> TSIG_OL
  function onEdit(e) {
    e.stopPropagation();
    e.preventDefault(); 
    var l = self._getLayerForLI(this.parentNode.parentNode);
    self.onedit(l); 
    self.dispatchEvent({ type: "ts_edit", layer: l });
  }
  ...
  for (var i=layers.length-1; i>=0; i--) {
    ...
    // Modificado: Añadida la clase workLayer y el evento click:setWorkLayer // ====> TSIG_OL
    var li = ol_ext_element.create('LI', {
      className: (layer.workLayer?"workLayer ":" ")+(layer.getVisible()?"visible ":" ")+(layer.get('baseLayer')?"baselayer":""),
      parent: ul,
      click: setWorkLayer
    });
    ...
    // Edit button   // ====> TSIG_OL
    if (this.onedit && layers[i].editable) {
      ol_ext_element.create('DIV', {
        className: 'layeredit',
        title: this.tip.edit,
        click: onEdit,
    ...
    else if (layer instanceof ts_layer_scale) li.classList.add('ts-layer-scale');  // ====> TSIG_OL 
